...
public class UserAPIProxy : IUserRepository
{
    ...
    public async Task<User?> Login(string emailOrUsername, string password)
    {
        ...
        var response = await this.httpClient.PostAsJsonAsync("api/users/login", loginData, JsonOptions);
    ...
    public async Task<bool> Register(User newUser)
    {
        ...
        var response = await this.httpClient.PostAsJsonAsync("api/users/register", newUser, JsonOptions);
